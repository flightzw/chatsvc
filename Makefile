...
ifeq ($(GOHOSTOS), windows)
	...
	ERROR_PROTO_FILES=$(shell $(Git_Bash) -c "find api/*/errno -name *.proto")
	VALIDATE_PROTO_FILES=$(shell $(Git_Bash) -c "find api/*/v* -name *.proto")
else
	INTERNAL_PROTO_FILES=$(shell find internal -name *.proto)
	API_PROTO_FILES=$(shell find api -name *.proto)
	ERROR_PROTO_FILES=$(shell find api/*/errno -name *.proto)
	VALIDATE_PROTO_FILES=$(shell find api/*/v* -name *.proto)
endif
...
data:
	gentool -dsn "root:123456@tcp(localhost:3306)/chatsvc?charset=utf8mb4&parseTime=True&loc=Local" \
